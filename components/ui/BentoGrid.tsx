...
export const BentoGridItem = ({
  ...
  const handleCopy=()=>{
    navigator.clipboard.writeText('kritsimarsingh@gmail.com');
  ...
  return (
    <div
      ...
      style={{background:'rgb(2,0,36)',backgroundColor:'linear-gradient(90deg,rgba(2,0,36,1) 0%,rgba(59,59,68,1) 26%, rgba(93,108,111,1) 100%)',}}
    >
        ...
        <div className="font-sans font-bold text-lg lg:text-3xl max-w-96 z-10">
          ...
          {/* here is the space for id=2 */}
            {id ===3 && (
              <div className="flex gap-1 lg:gap-5 w-fit absolute -right-1 lg:-right-0">
                <div className="flex flex-col gap-3 lg:gap-8">
                  {['Centralization','Incentives','Ownership'].map((item)=>(
                ...
                <div className="flex flex-col gap-3 lg:gap-8">
                <span className="py-4 px-1 rounded-lg text-center bg-[#10132e]"/>

                  {['Decentralization','AI','Crypto'].map((item)=>(
                    <span key={item} className="py-2 lg:py4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                      {item}
                    </span>
                  ))}
